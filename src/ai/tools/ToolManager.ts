...
import { ToolCallback, ToolConfig, ToolContext } from './ToolTypes';
...
export class ToolManager {
  ...
  static async executeTool(
    ...
    if (tool) {
      ...
      console.info('Executing tool', name, 'with parameters', values);
      return await tool.implementation.execute(context, ...values);
    } else {
      console.warn(`Tool ${name} not found in ToolManager.tools. parameters: ${JSON.stringify(parameters)}`);
    }
  }
}
